import Foundation
@_spi(MainActorUtilites) import _Concurrency
...
extension MainActor {
    ...
    public static func runAsap<Success>(
        ...
        if Thread.isMainThread, #available(iOS 17, macOS 14, *) {
            ...
            Task(priority: priority) { @MainActor in await body() }
        }
    }
    ...
    public static func runAsap<Success>(
        ...
        if Thread.isMainThread, #available(iOS 17, macOS 14, *) {
            ...
            Task(priority: priority) { @MainActor in try await body() }
        }
    }
...
extension Task where Failure == Never {
    ...
    static func startOnMainActor(
        priority: TaskPriority? = nil,
        @_inheritActorContext @_implicitSelfCapture _ work: consuming @Sendable @escaping @MainActor
        () async -> Success
    ) -> Task<Success, Never>
}
...
extension Task where Failure == Error {
    @_silgen_name(
        "$sScTss5Error_pRs_rlE16startOnMainActor8priority_ScTyxsAA_pGScPSg_xyYaYbKScMYccntFZ")
    @available(macOS 14.0, iOS 17.0, watchOS 10.0, tvOS 17.0, *)
    @MainActor
    @discardableResult
    static func startOnMainActor(
