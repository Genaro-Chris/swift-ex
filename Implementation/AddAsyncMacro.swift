...
public struct AddAsyncMacro: PeerMacro {
    ...
    >(
        ...
        switch funcDecl.signature.returnClause?.type.with(\.leadingTrivia, []).with(
            \.trailingTrivia, [])
        {
        ...
        guard
            let completionHandlerParameterAttribute = funcDecl.signature.parameterClause.parameters
                .last?.type.as(AttributedTypeSyntax.self),
        ...
        guard
            completionHandlerParameter.returnClause.type.with(\.leadingTrivia, []).with(
                \.trailingTrivia, []
            ).description == "Void"
                || completionHandlerParameter.returnClause.type.with(\.leadingTrivia, []).with(
                    \.trailingTrivia, []
            ...
            throw CustomError.message(
                "@addAsync requires an function that has a completion handler that returns Void or nothing"
            )
        }
                ...
                funcDecl.signature
                    ...
                    .with(
                        ...
                        successReturnType != nil
                            ? ReturnClauseSyntax(
                                leadingTrivia: .space,
                                type: successReturnType!.with(\.leadingTrivia, .space)) : nil
                    )  // add result type
                    .with(
                        \.parameterClause,
